...
int32_t cal_step3(void)
{
	...
	puts("\nVPP-Calibration: \n");
    //die huehere Spannung muss im ADC-Meuebereich liegen!!
	...
	if (Uhigh <= U10)
	{
        ...
        prog.d_gain_off  = (pwm-10) / (Uhigh-U10);			// pwm-uenderung fuer 1V
    ...
    //die huehere Spannung muue im ADC-Meuebereich liegen!!
	...
	if (Uhigh <= U10)
	{
        ...
        prog.d_gain_on  = (pwm-10) / (Uhigh-U10);			// pwm-uenderung fuer 1V
...
float vpp_getADC(int32_t kanal, int32_t zyklen)
{
	...
	if (count)
	{
		Volt = Volt / count;
		Result = Volt;
        prog.ADCL = (uint8_t)((uint16_t)(floor(Volt + 0.5)) % 0x100);
        prog.ADCH = (uint8_t)((uint16_t)(floor(Volt + 0.5)) / 0x100);
	} else {
		prog.ADCL = 0;
		prog.ADCH = 0;
	};
...
// wartet max 500 ms auf Stabilituet
...
//dabei muue pwm begrenzt werden auf 10 .. 70
...
int32_t vpp_setVpp(float VppSoll)
{
	...
	//      = 4,7V * 120 /(120-pwm)
	//
    // 1V entspricht einer PWM-uenderung um 2,6
    // 1V entspricht einer PWM-uenderung um 5,8 unter Last
