...
int32_t search_brenner8(void)
{
	...
	usb_dev_handle *udev;
    int32_t mode, usbmode, ep_in, ep_out;
...
word HexinMux(int32_t Adr)
{
	...
	if ((Adr >= prog.pic.calmem.min) && (Adr <= prog.pic.calmem.max))
	{
		...
		//an dieser Stelle kann ein manuell eingestellter OSCCAL eingef�gt werden
		// prog.OsccalPar enth�lt 0..63 (PIC12) oder -64..63 (PIC10)
